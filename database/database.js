...
records.getSongsAndAlbumsOfArtists = () => db.any('SELECT artist.name AS artist, song.title AS song, album.title AS album FROM artists artist INNER JOIN albums album ON album.artist_id = artist.id INNER JOIN songs song ON song.album_id = album.id GROUP BY song.title, album.title, artist.name ORDER BY artist.name;')
